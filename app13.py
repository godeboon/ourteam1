...
# 전체 레이아웃을 컨테이너로 감싸기
with st.container():  # with 절 : 하나의 기능을 수행하는 코드로 묶어주는 역할 (가독성 높이기)
    ...
    with col1: 
        st.header("원본 영상") # 영상이 업로드가 되었다면 
        if uploaded_file is not None:
            st.video(uploaded_file)  # 재생하라 !
        else:
            st.write("원본 영상을 표시하려면 비디오 파일을 업로드하세요.")

    with col2:  # col2 에 해당하는 영역의 제목 
        st.header("사물 검출 결과 영상") 
        if "processed_video" in st.session_state: # 사물검출 완료된 비디오가 있으면 
            st.video(st.session_state["processed_video"]) # 그 비디오를 플레이하라! 
        else:
